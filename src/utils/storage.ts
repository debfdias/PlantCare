...
export async function registerPlant(plant: PlantProps): Promise<void> {
  try {
    ...
    const notificationID = await Notifications.scheduleNotificationAsync({
      content: {
        title: 'Heeeey 🌱',
        body: `Time to watering your ${plant.name}`,
        sound: true,
    ...
    await AsyncStorage.setItem('@plantcare:plants',
      JSON.stringify({
        ...newPlant,
        ...oldPlants
      }));
  ...
  try {
    ...
    .sort((a, b) => 
      Math.floor(
        ...
        (Math.floor(Number(new Date(b.dateTimeNotification).getTime) / 1000)) 
      )
    );
